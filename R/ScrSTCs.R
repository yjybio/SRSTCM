...
#'
#' @return if data is scRNA-seq: 1) scRNA-seq quality test results; 2) scRNA-seq cluster map; 3) aneuploid/diploid prediction results; 4)senescent tumor cells prediction results; 5)annotated cluster diagram of senescent tumor cells; 6)differential genes in senescent and non-senescent tumor cell clusters; 7)expression of the top 10 differentially expressed genes in senescent and non-senescent tumor cell clusters; if data is bulk: 1)the aging score results; 2)survival outcomes in the high and low aging score groups.
#' @export
#'
#' @examples
                     ...
                     win.size = 25, KS.cut = 0.15, distance = "euclidean", n.cores = 1)
{
  ...
  if (type == "sc") {
    ...
    if (cancer == "LUNG") {
      copykat.prediction <- copykat.prediction[which(rownames(copykat.prediction) %in% LUNG.aging.gene), ]
      dir <- which(LUNG.aging.gene %in% rownames(copykat.prediction))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        copykat.prediction <- copykat.prediction[match(LUNG.aging.gene[dir], rownames(copykat.prediction)), ]
        LUNG.aging.average <- LUNG.aging.average[dir]
        LUNG.no.aging.average <- LUNG.no.aging.average[dir]
        p.no.aging <- ( LUNG.no.aging.average + 1 ) / ( sum(LUNG.no.aging.average) + length(LUNG.no.aging.average) )
        p.aging <- ( LUNG.aging.average + 1 ) / ( sum(LUNG.aging.average) + length(LUNG.aging.average) )
        q.LUNG <- apply(copykat.prediction, 2, q)
        aging <- c()
        for (i in 1 : ncol(q.LUNG)) {
          if (argmax(p.aging, q.LUNG[, i]) > argmax(p.no.aging, q.LUNG[, i])) {
            aging <- c(aging, colnames(q.LUNG)[i])
          }
        }
        write.table(aging, paste(cancer, "_STC_prediction.txt", sep = ""), sep = "\t", row.names=FALSE, col.names = FALSE, quote = FALSE)
      }
    ...
    if (cancer == "SKCM") {
      copykat.prediction <- copykat.prediction[which(rownames(copykat.prediction) %in% SKCM.aging.gene), ]
      dir <- which(SKCM.aging.gene %in% rownames(copykat.prediction))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        copykat.prediction <- copykat.prediction[match(SKCM.aging.gene[dir], rownames(copykat.prediction)), ]
        SKCM.aging.average <- SKCM.aging.average[dir]
        SKCM.no.aging.average <- SKCM.no.aging.average[dir]
        p.no.aging <- ( SKCM.no.aging.average + 1 ) / ( sum(SKCM.no.aging.average) + length(SKCM.no.aging.average) )
        ...
        for (i in 1 : ncol(q.SKCM)) {
          if (argmax(p.aging, q.SKCM[, i]) > argmax(p.no.aging, q.SKCM[, i])) {
            aging <- c(aging, colnames(q.SKCM)[i])
          }
        }
        write.table(aging, paste(cancer, "_STC_prediction.txt", sep = ""), sep = "\t", row.names=FALSE, col.names = FALSE, quote = FALSE)
      }
    ...
    if (cancer == "LIHC") {
      copykat.prediction <- copykat.prediction[which(rownames(copykat.prediction) %in% LIHC.aging.gene), ]
      dir <- which(LIHC.aging.gene %in% rownames(copykat.prediction))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) >1) {
        copykat.prediction <- copykat.prediction[match(LIHC.aging.gene[dir], rownames(copykat.prediction)), ]
        LIHC.aging.average <- LIHC.aging.average[dir]
        LUNG.no.aging.average <- LUNG.no.aging.average[dir]
        p.no.aging <- ( LUNG.no.aging.average + 1 ) / ( sum(LUNG.no.aging.average) + length(LUNG.no.aging.average) )
        p.aging <- ( LIHC.aging.average + 1 ) / ( sum(LIHC.aging.average) + length(LIHC.aging.average) )
        q.LIHC <- apply(copykat.prediction, 2, q)
        aging <- c()
        for (i in 1 : ncol(q.LIHC)) {
          if (argmax(p.aging, q.LIHC[, i]) > argmax(p.no.aging, q.LIHC[, i])){
            aging <- c(aging, colnames(q.LIHC)[i])
          }
        }
        write.table(aging, paste(cancer, "_STC_prediction.txt", sep = ""), sep = "\t", row.names=FALSE, col.names = FALSE, quote = FALSE)
      }
    ...
    if (cancer == "BRCA") {
      copykat.prediction <- copykat.prediction[which(rownames(copykat.prediction) %in% BRCA.aging.gene), ]
      dir <- which(BRCA.aging.gene %in% rownames(copykat.prediction))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        copykat.prediction <- copykat.prediction[match(BRCA.aging.gene[dir], rownames(copykat.prediction)), ]
        BRCA.aging.average <- BRCA.aging.average[dir]
        BRCA.no.aging.average <- BRCA.no.aging.average[dir]
        p.no.aging <- ( BRCA.no.aging.average + 1 ) / ( sum(BRCA.no.aging.average) + length(BRCA.no.aging.average) )
        p.aging <- ( BRCA.aging.average + 1 ) / ( sum(BRCA.aging.average) + length(BRCA.aging.average) )
        q.BRCA <- apply(copykat.prediction, 2, q)
        aging <- c()
        for (i in 1 : ncol(q.BRCA)) {
          if (argmax(p.aging, q.BRCA[, i]) > argmax(p.no.aging, q.BRCA[, i])) {
            aging <- c(aging, colnames(q.BRCA)[i])
          }
        }
        write.table(aging, paste(cancer, "_STC_prediction.txt", sep = ""), sep = "\t", row.names=FALSE, col.names = FALSE, quote = FALSE)
      }
  ...
  if (type == "bulk") {
    print("Read and analyze bulk Data ...")
    name <- rownames(data)
    ...
    if (cancer == "LUNG") {
      data <- data[which(rownames(data) %in% LUNG.aging.gene), ]
      dir <- which(LUNG.aging.gene %in% rownames(data))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        data <- data[match(LUNG.aging.gene[dir], rownames(data)), ]
        LUNG.aging.average <- LUNG.aging.average[dir]
        p.aging <- ( LUNG.aging.average + 1 ) / ( sum(LUNG.aging.average) + length(LUNG.aging.average) )
        q.LUNG <- apply(data, 2, q)
        Aging.score <- matrix(nrow = ncol(q.LUNG), ncol = 2)
        for (i in 1 : ncol(q.LUNG)) {
          Aging.score[i, 1] <- colnames(q.LUNG)[i]
          Aging.score[i, 2] <- argmax(p.aging, q.LUNG[, i])
        }
        colnames(Aging.score) <- c("sample", "aging_score")
        Aging.score <- as.data.frame(Aging.score)
        write.table(Aging.score, paste(cancer, "_aging_score.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
    ...
    if (cancer == "LIHC") {
      data <- data[which(rownames(data) %in% LIHC.aging.gene), ]
      dir <- which(LIHC.aging.gene %in% rownames(data))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) >1) {
        data <- data[match(LIHC.aging.gene[dir], rownames(data)), ]
        LIHC.aging.average <- LIHC.aging.average[dir]
        p.aging <- ( LIHC.aging.average + 1 ) / ( sum(LIHC.aging.average) + length(LIHC.aging.average) )
        q.LIHC <- apply(data, 2, q)
        Aging.score <- matrix(nrow = ncol(q.LIHC), ncol = 2)
        for (i in 1 : ncol(q.LIHC)) {
          Aging.score[i, 1] <- colnames(q.LIHC)[i]
          Aging.score[i, 2] <- argmax(p.aging, q.LIHC[, i])
        }
        colnames(Aging.score) <- c("sample", "aging_score")
        Aging.score <- as.data.frame(Aging.score)
        write.table(Aging.score, paste(cancer, "_aging_score.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
    ...
    if (cancer == "BRCA") {
      data <- data[which(rownames(data) %in% BRCA.aging.gene), ]
      dir <- which(BRCA.aging.gene %in% rownames(data))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        data <- data[match(BRCA.aging.gene[dir], rownames(data)), ]
        BRCA.aging.average <- BRCA.aging.average[dir]
        p.aging <- ( BRCA.aging.average + 1 ) / ( sum(BRCA.aging.average) + length(BRCA.aging.average) )
        q.BRCA <- apply(data, 2, q)
        Aging.score <- matrix(nrow = ncol(q.BRCA), ncol = 2)
        for (i in 1 : ncol(q.BRCA)) {
          Aging.score[i, 1] <- colnames(q.BRCA)[i]
          Aging.score[i, 2] <- argmax(p.aging, q.BRCA[, i])
        }
        colnames(Aging.score) <- c("sample", "aging_score")
        Aging.score <- as.data.frame(Aging.score)
        write.table(Aging.score, paste(cancer, "_aging_score.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
    ...
    if (cancer == "OVCA") {
      data <- data[which(rownames(data) %in% OVCA.aging.gene), ]
      dir <- which(OVCA.aging.gene %in% rownames(data))
      if (length(dir) == 0 || length(dir) == 1)
        stop("too few aging marker genes;cannot be calculated")
      if (length(dir) > 1) {
        data <- data[match(OVCA.aging.gene[dir], rownames(data)), ]
        OVCA.aging.average <- OVCA.aging.average[dir]
        p.aging <- ( OVCA.aging.average + 1 ) / ( sum(OVCA.aging.average) + length(OVCA.aging.average) )
        q.OVCA <- apply(data, 2, q)
        Aging.score <- matrix(nrow = ncol(q.OVCA), ncol = 2)
        for (i in 1 : ncol(q.OVCA)) {
          Aging.score[i, 1] <- colnames(q.OVCA)[i]
          Aging.score[i, 2] <- argmax(p.aging, q.OVCA[, i])
        }
        colnames(Aging.score) <- c("sample", "aging_score")
        Aging.score <- as.data.frame(Aging.score)
        write.table(Aging.score, paste(cancer, "_aging_score.txt", sep = ""), sep = "\t", row.names = FALSE, quote = FALSE)
    ...
    if (!is.null(survival)) {
      ...
      pdf(paste(cancer, "_aging_sur.pdf", sep = ""), height = 6, width = 8, onefile = FALSE)
      p5 <- ggsurvplot(
        fit,
        data = survival,
